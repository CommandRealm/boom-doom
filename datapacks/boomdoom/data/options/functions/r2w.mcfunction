...
setblock 10 67 25 oak_wall_sign[facing=north]{front_text:{messages:["{\"text\":\"Rounds\",\"bold\":false,\"color\":\"#3f944b\"}", "{\"text\":\"to win:\",\"color\":\"#3f944b\",\"bold\":false}", "{\"score\":{\"objective\":\"r2w\",\"name\":\"rounds\"},\"color\":\"dark_green\",\"bold\":true,\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/execute if score game game matches 1.. run function options:error\"}}", '{"text":"<<<         >>>","color":"#3cff00","bold":true,"clickEvent":{"action":"run_command","value":"/execute as @s at @s anchored eyes if score game game matches 0 run function options:click_r2w"}}']}} replace

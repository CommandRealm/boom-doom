...
##What sets this off:
execute as @e[type=armor_stand,tag=throwable,scores={attack=4..}] at @s if entity @e[tag=mob,distance=..3] run scoreboard players set @s attack 60
scoreboard players set @e[type=armor_stand,tag=throwable,scores={attack=4..},nbt={OnGround:1b}] attack 60
execute as @e[type=armor_stand,tag=throwable,scores={attack=50..}] at @s run function items:whirlpool/activate
